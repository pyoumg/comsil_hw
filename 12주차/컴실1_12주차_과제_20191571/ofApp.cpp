...
void ofApp::bfsdraw() {
	...
	temp3 = searchshort[maze_row*maze_col - 1];
	temp = front;
	ofSetColor(0, 100, 0);
	store_pointer bfsstart = bfsstore;
	while (bfsstart != NULL) {
		ofDrawLine(bfsstart->vertex1 % maze_row * 40 + 20, bfsstart->vertex1 / maze_row * 40 + 20, bfsstart->vertex2 % maze_row * 40 + 20, bfsstart->vertex2 / maze_row * 40 + 20);
		bfsstart = bfsstart->link;
	}
	ofSetColor(100, 0, 0);
	while (temp3->left != NULL) {
		ofDrawLine(temp3->vertex % maze_row * 40 + 20, temp3->vertex / maze_row * 40 + 20, temp3->left->vertex  % maze_row * 40 + 20, temp3->left->vertex / maze_row * 40 + 20);
		temp3 = temp3->left;
	}
...
bool ofApp::BFS() {
	node_pointer w;
	store_pointer Nstore, Prestore;
	short_pointer newshort, preshort;
	searchshort = (short_pointer*)malloc(sizeof(short_pointer)*maze_row*maze_col);
	for (int i = 0; i < maze_col*maze_row; i++) {
		searchshort[i] = (short_pointer)malloc(sizeof(shortbfs));
		searchshort[i]->left = NULL;
		searchshort[i]->right = NULL;
		searchshort[i]->vertex = i;
	}
	bfsstore = NULL;
	int v;
	front = rear = NULL;
		...
		for (w = graph[v]; w; w = w->link) {
			...
			if (!visited[w->vertex]) {
				...
				if (v != w->vertex) {
					...
					Nstore = (store_pointer)malloc(sizeof(store));
					Nstore->vertex1 = v;
					Nstore->vertex2 = w->vertex;
					Nstore->print_flag = true;
					if (bfsstore == NULL) {
						bfsstore = Nstore;
						bfsstore->link = NULL;
						Prestore = Nstore;
						...
						Prestore->link = Nstore;
						Nstore->link = NULL;
						Prestore = Nstore;
					}
				}
				searchshort[v]->right = searchshort[w->vertex];
				searchshort[w->vertex]->left = searchshort[v];
				addq(w->vertex);
				visited[w->vertex] = 1;
			}
			if (w->vertex == maze_col * maze_row - 1)
				return true;
		}
	}
	return false;
